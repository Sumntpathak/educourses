    ...
    <div>
      ...
      <div className="shdr">
        <div>
          ...
          <div className="ssub">Mark daily attendance by batch</div>
        </div>
      </div>
      ...
      <div className="form-card">
        ...
        {selBatch && bStu.length > 0 && (
          <div style={{ marginTop: '12px', fontSize: '12px', display: 'flex', gap: '16px' }}>
            <span className="badge present">Present: {pc}</span>
            <span className="badge absent">Absent: {ac}</span>
        ...
        <div>
          <div className="table-wrap">
            <table style={{ minWidth: 'auto' }}>
              <thead>
                <tr><th>#</th><th>Student</th><th style={{ textAlign: 'center' }}>Present</th><th style={{ textAlign: 'center' }}>Absent</th><th style={{ textAlign: 'center' }}>Late</th></tr>
              </thead>
              <tbody>
                {bStu.map((s, i) => {
                    ...
                    <tr key={s.id} style={{ background: st === 'Absent' ? 'rgba(239,68,68,.03)' : '' }}>
                      ...
                      {['Present', 'Absent', 'Late'].map(v => (
                        <td key={v} style={{ textAlign: 'center' }}>
                          <input type="radio" name={`a-${s.id}`} checked={st === v}
          ...
          {msg && (
            <div style={{
              ...
              color: msg.type === 'error' ? 'var(--danger)' : 'var(--accent3)',
