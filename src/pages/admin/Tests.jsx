...
export default function Tests() {
  ...
  useEffect(() => {
    Promise.all([coaching_getTests(), coaching_getBatches(), coaching_getStudents()])
      .then(([t, b, s]) => { setTests(t.tests || []); setBatches(b.batches || []); setStudents(s.students || []); setLoading(false); })
      ...
      {showForm && (
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="fg"><label className="fl">Batch <span className="req">*</span></label>
              <select className="fi" value={selBatch} onChange={e => onBatch(e.target.value)}>
                <option value="">Select</option>
                {batches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <div className="fg"><label className="fl">Test Name <span className="req">*</span></label><input className="fi" value={form.testName} onChange={e => setForm(f => ({ ...f, testName: e.target.value }))} /></div>
            ...
            <div className="fg"><label className="fl">Type</label>
              <select className="fi" value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}>
                <option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="mock">Mock</option>
              </select>
            </div>
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <div className="table-wrap">
                <table style={{ minWidth: 'auto' }}>
                  <thead>
                    <tr><th style={{ width: '40px' }}>#</th><th>Student</th><th style={{ textAlign: 'center' }}>Marks (/{form.maxMarks})</th><th style={{ textAlign: 'center' }}>Absent</th></tr>
                  </thead>
                  <tbody>
                    {batchStu.map((s, i) => {
                      const isAb = !!absent[s.id];
                      return (
                        <tr key={s.id} style={{ opacity: isAb ? .5 : 1 }}>
                          <td style={{ color: 'var(--muted)' }}>{i + 1}</td>
                          <td style={{ fontWeight: 500 }}>{s.name}</td>
                          <td style={{ textAlign: 'center' }}>
                            <input type="number" min={0} max={form.maxMarks} value={isAb ? '' : marks[s.id] ?? ''} disabled={isAb}
                              onChange={e => { setMarks(m => ({ ...m, [s.id]: e.target.value })); setAbsent(a => ({ ...a, [s.id]: false })); }}
                              className="fi" style={{ width: '70px', textAlign: 'center', padding: '6px', minHeight: 'auto' }} />
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <input type="checkbox" checked={isAb}
          ...
          <details key={t.id || ti} className="form-card" style={{ overflow: 'hidden', cursor: 'pointer' }}>
            ...
            <div style={{ paddingTop: '14px' }}>
              <div className="table-wrap">
                <table style={{ minWidth: 'auto' }}>
                  <thead>
                    <tr><th style={{ width: '40px' }}>Rank</th><th>Student</th><th style={{ textAlign: 'center' }}>Marks</th><th style={{ textAlign: 'center' }}>%</th><th style={{ textAlign: 'center' }}>Grade</th></tr>
                  </thead>
                  <tbody>
                    {[...(t.scores || [])].sort((a, b) => (b.obtained || 0) - (a.obtained || 0)).map((sc, i) => {
                      ...
                      return (
                        <tr key={i} style={{ background: i < 3 && !sc.absent ? ['rgba(245,158,11,.06)', 'rgba(192,192,192,.04)', 'rgba(205,127,50,.04)'][i] : '' }}>
                          ...
                          <td style={{ textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", color: sc.absent ? 'var(--muted)' : parseFloat(p) >= 60 ? 'var(--accent3)' : 'var(--danger)' }}>
